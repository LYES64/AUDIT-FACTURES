...
// ---- surcharges manuelles "Posé ?" (mémorisées sur le poste) ----
const OVK="audit_pose_ovr_v1";
...
function recompFstat(x){
 if(x.invoice&&x.invoice.trim())return "Facturé";
...
function applyOvr(){DATA.dossiers.forEach(x=>{const k=dkey(x);
  if(OVR[k]!==undefined){x.pose=OVR[k];x.fstat=recompFstat(x);x._ovr=true;}else{x._ovr=false;}});}
applyOvr();
...
function dossTable(nonfacView){
 ...
 let body=d.map(x=>{const pid=x.proof||proofOf(x.invoice);
   const facCell=x.invoice?`<b>${x.invoice}</b>${pid?` <a class=plink href="#" onclick="goProof('${pid}');return false" title="Voir la preuve">📎</a>`:""}`:"<span class=small>—</span>";
   return `<tr>
